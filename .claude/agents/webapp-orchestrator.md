...
8. **Sempre, dopo lo step 7**: esegui `node ace/scripts/check_threshold.js reflector` per vedere se le trace non ancora processate in `ace/traces/` hanno raggiunto la soglia configurata in [`ace/config/thresholds.json`](../../ace/config/thresholds.json).
...
- Il commit e la PR del codice applicativo sono compito dei subagent (hanno `Bash`/`execute` per questo), non tuo: il tuo `Bash` in questo workflow serve solo per il ciclo ACE (step 7-8), non per operazioni git sul codice applicativo.
