...
`vscode/askQuestions` — non limitarti a scrivere la domanda come testo
...
`vscode/askQuestions` non è invocabile, dillo esplicitamente e poni la
...
4. **Checklist di conflitto semantico (non automatizzabile, vedi
   [gate.js](../scripts/gate.js))**: per ogni decisione `ADD`/`UPDATE` che
...
5. **STOP — chiedi conferma esplicita con `vscode/askQuestions`**:
...
7. **STOP — chiedi conferma esplicita con `vscode/askQuestions`**: "Il
