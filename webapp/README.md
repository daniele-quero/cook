...
Per una ricetta, l'endpoint cerca prima una PR di segnali ancora aperta nel repository GitHub e ne verifica il branch associato. Se non trova una PR, cerca anche un branch di segnali esistente con l'identificatore macchina `chat-signals/v1/<slug-ricetta>/<id-esadecimale>`; se esiste, scrive lì e apre una PR per renderlo revisionabile. Solo quando non esistono né una PR valida né un branch identificato per la ricetta crea un nuovo branch con quel formato dal branch predefinito. L'identificazione usa marker macchina nella PR e nel ref; conserva la compatibilità con le PR create in precedenza tramite il loro titolo, senza dedurre l'identità da un nome di branch legacy.

Le nuove PR hanno titolo `chore(signal-log): <slug-ricetta> — raccolti il <YYYY-MM-DD HH:mm UTC>`. Quando una PR viene riusata, il suo titolo non viene modificato: data e ora restano quindi quelle del primo segnale raccolto dalla PR. Il rilevamento delle PR aperte resta compatibile anche con i titoli generati nel formato precedente `chore(chat-signals): segnali per <slug-ricetta> (<timestamp ISO UTC>)`; PR di codice, come `feat(chat-signals): …`, non sono segnali e non vengono riusate. `trace_persistence` include `branch`, `pull_request_url` e `reused` in caso di successo; in caso di errore include `step` e lo status HTTP GitHub, quando disponibile. Una PR deve essere approvata e integrata prima che il trace sia disponibile per gli script di revisione sul branch principale. Se la scrittura o la creazione della PR fallisce dopo la creazione del branch, il branch resta disponibile per una verifica manuale e la risposta e' `502`.
