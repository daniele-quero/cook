...
tools: [read, search/codebase, edit, execute, agent]
...
<!-- ASSET-SYNC:BEGIN — generato automaticamente, non modificare a mano tra questi marker -->
  - source: .claude/agents/cook-signals-reviewer.md
  - original-tools-claude: Read, Glob, Edit, Bash, Agent
...
1. Se almeno un segnale del gruppo ha `recipe_scope: "current_recipe"`, leggi con `read` il file [`webapp/recipes/<recipe_slug>.md`](../../webapp/recipes) corrispondente (risolvi con `search/codebase` se necessario).
...
Dopo aver completato la valutazione di un trace file (indipendentemente dall'esito), archivialo con `execute`:
