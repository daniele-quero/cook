...
agents: [Webapp-frontend, Webapp-backend]
...
- Il commit e la PR del codice applicativo sono compito dei subagent (hanno `execute`/`execute` per questo), non tuo: il tuo `execute` in questo workflow serve solo per il ciclo ACE (step 7-8), non per operazioni git sul codice applicativo.
